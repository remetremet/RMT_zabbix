...
#       echo ${now_seconds}
  ...
  ;;
  -i)
     issue_dn=`openssl s_client -servername ${servername} -host ${host} -port ${port} -showcerts ${starttls} -prexit </dev/null 2>/dev/null | sed -n '/BEGIN CERTIFICATE/,/END CERT/p' | openssl x509 -text 2>/dev/null | sed -n 's/ *Issuer: *//p'`
     if [ -n "${issue_dn}" ]; then
